...
%token INT EGALE FIN PV PLUS MOINS FOIS DIVISER AFFICHER PAR_G PAR_D PP PG PE GE EE POUR STR INCREMENT DECREMENT SI SINON AC_G AC_D
%token <entier> ENTIER
...
Input : 
      | Input Ligne 
      | Input error FIN { //quand il ne reconnait pas la ligne, ne se ferme pas mais affiche un message
            yyerrok;
        }
      ;
...
Ligne : Affectation {
      ...
      | Expression PV {if($1.type == 0) printf("--> %d\n", $1.vInt);else printf("--> %s\n", $1.vStr);}
      | Structure 
      | Fonction
      | FIN 
      ;
...
Affectation : STR IDENT EGALE Expression PV { 
    ...
    | IDENT EGALE Expression PV { 
          ...
          if(pos != -1) { // on verifie que le symbole existe
              if(strcmp(table_symboles[pos].type, "INT") == 0 && $3.type == 0) // si la variable est un entier et que le type de l'expression est un entier, alors on peut inserer.
                  insererEntier($1, $3.vInt);
              else if(strcmp(table_symboles[pos].type, "STRING") == 0 && $3.type == 1) // pareil mais pour les string
...
Structure: POUR PAR_G Affectation Expression PV Affectation PAR_D {printf("Structure de la boucle FOR correcte.\n");}
	| SI PAR_G Expression PAR_D {printf("Structure IF correcte.\n");}
	| POUR PAR_G Affectation Expression PV Affectation PAR_D AC_G LignesMultiples AC_D {printf("Structure de la boucle FOR correcte.\n");}
	| SI PAR_G Expression PAR_D AC_G LignesMultiples AC_D SINON AC_G LignesMultiples AC_D {printf("Structure IF/ELSE correcte.\n");};

LignesMultiples: LignesMultiples Ligne; // pour accepter les lignes dans les if
		| ;
